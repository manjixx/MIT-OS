...
如果没有操作系统会发生什么？通过稻草人提案法（即通过头脑风暴找出缺点）来考虑一下，如果没有操作系统，或者操作系统只是一些库文件，比如，在使用`Python`时，通过`import os` 就可以将整个操作系统加载到应用程序中。此时，有一个`Shell`，并且引用了代表操作系统的库。同时有一些其他的应用程序，例如`echo`。
...
**如果没有操作系统，应用程序会直接与硬件交互**。比如，应用程序可以直接看到CPU的多个核、磁盘与内存。应用程序和硬件资源之间没有一个额外的抽象层，如下图所示。
